...
namespace Motwane_UVSS
{
    ...
    public partial class Main_uvss_page : System.Windows.Window
    {
       ...
       // private const string driversnapshotUrl = "https://192.168.4.57/cpapi/snapshot.cgi";
        ...
        VehicleEntryLog current;

        public Main_uvss_page(VehicleEntryLog v)
        {
            InitializeComponent();
           ...
           // Task.Run(() => StartdriverCamera()); // Run in background thread

            // nvrcam.Show();
            this.current = v;    
            Sticked_image.Source = v.UndersideImage;
            Driver_image.Source = v.DriverCamImage;
            Anpr_image.Source = v.AnprImage;
            Numberplate_number_box.Text = v.Numberplate;
            ...
            {
               // lblImageCount.Text = "Camera ready.";
            ...
            {
                // CP Plus typically uses "cam/realmonitor?channel=1&subtype=0"
                var rtsp = $"rtsp://{CameraUser}:{CameraPass}@{ip}:554/cam/realmonitor?channel=1&subtype=0";
                RtspUrls.Add(rtsp);
            }

            // ▶︎ Initialize LibVLC (will automatically look under "libvlc\win-x64" from NuGet)
            Core.Initialize();
            _libVLC = new LibVLC();
        ...
        private void PASS_BTN_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand($"Select Top(1) [remark] from vehicle_entry_log where [numberplate]= '{numberplate}' order by Concat(entry_date,[entry_time]) desc", conn);
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    try
                    {
                        if (v != "Normal" || v != null || v != "")
                        {
                            if (System.Windows.Forms.MessageBox.Show($"The Registered Vehicle no is {dt.Rows[0][0].ToString()}", "Will You allow this Operation", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.No)
                            {
                                return;
                            }
            ...
            //  SaveRecordTovideo_management_Database(Numberplate_number_box.Text, @"D:\uvss\under_vehicle_video_center\camera1.mp4", @"D:\uvss\under_vehicle_video_center\camera1.mp4", @"D:\uvss\under_vehicle_video_center\camera1.mp4");

            try 
            {
                SaveRecordTovideo_management_Database(Numberplate_number_box.Text, pinhole_came_path + @"\camera1.mp4", pinhole_came_path + @"\camera2.mp4", pinhole_came_path + @"\camera3.mp4");
            }
            catch { }
            Numberplate_number_box.Text = "";
       ...
       // public bool aic_on = false;
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        ...
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                // --- FINAL INTEGRATION ---
                                AicViewerWindow viewerWindow = new AicViewerWindow();

                                // CHANGE 3: Pass the new timestamp string to the viewer window
                                viewerWindow.LoadAndCompareImages(sourceImagePath, dbImagePath, timestampInfo);
        ...
        {
            // PrintPreviewCurrentWindow(this);
            // OpenPrintPreview(this);

            //SaveScreenAsPdf1(this,"D:\\gg.pdf");

            try
            {


                if(Numberplate_number_box.Text =="" || Numberplate_number_box.Text == null)
                {
                    return;
                }
                if (current != null)
                {
                    SaveScreenAsPdf1(this, $@"D:\\Reports\\{current.EntryDate.ToString().Replace(':', ' ')} {current.EntryTime.ToString().Replace(':',' ')} {current.Numberplate}.pdf");
                    current = null;
                }
                else
                {
                    //SaveScreenAsPdf1(this, "D:\\gg.pdf");
                    SaveScreenAsPdf1(this, "D:\\Reports\\" + DateTime.Now.ToShortTimeString().Replace(':',' ') +" " + DateTime.Now.ToShortDateString().Replace(':', ' ') + ".pdf");
                }
            }
            catch (Exception)
            {
      ...
      public void PrintPreviewCurrentWindow(System.Windows.Window window)
    {
           
            {
                ...
                // Page size
                fixedDoc.DocumentPaginator.PageSize =
                ...
                // Clone the visual
                System.Windows.Media.VisualBrush visualBrush = new System.Windows.Media.VisualBrush(window)
                {
      ...
      public  void OpenPrintPreview(System.Windows.Window window)
      {
            System.Windows.Documents.FixedDocument doc = CreateDocumentFromCurrentScreen(window);
        ...
        System.Windows.Documents.FixedDocument CreateDocumentFromCurrentScreen(System.Windows.Window window)
        {
            System.Windows.Documents.FixedDocument doc = new System.Windows.Documents.FixedDocument();

            System.Windows.Size pageSize = new System.Windows.Size(816, 1056); // Letter @96 DPI
        ...
        private void SaveScreenAsPdf(UIElement element, string pdfPath)
        {
            try
            {

            
            // Step 1: Create FixedDocument
            System.Windows.Documents.FixedDocument doc = new System.Windows.Documents.FixedDocument();
            System.Windows.Size pageSize = new System.Windows.Size(816, 1056); // Letter
            doc.DocumentPaginator.PageSize = pageSize;
            ...
            // Step 2: Save as XPS temporarily
            string tempXps = System.IO.Path.Combine(/*System.IO.Path.GetTempPath(),*/ "temp.xps");
            using (XpsDocument xpsDoc = new XpsDocument(tempXps, FileAccess.Write))
            {
                XpsDocumentWriter writer = XpsDocument.CreateXpsDocumentWriter(xpsDoc);
                writer.Write(doc);
            }

            // Step 3: Convert XPS to PDF silently
            ConvertXpsToPdf(tempXps, pdfPath);
        ...
        private void ConvertXpsToPdf(string xpsFile, string pdfFile)
        {

            try
            {
                XPSToPdfConverter converter = new XPSToPdfConverter();
                var doc = new Aspose.Pdf.Document("temp.xps", new Aspose.Pdf.XpsLoadOptions());
                doc.Save(pdfFile);
            }
            catch (Exception ex)
            {
            ...
            //using (FileStream xpsStream = File.Open(xpsFile, FileMode.Open, FileAccess.Read))
            //{
            //    // Use the stream Convert method
            //    Syncfusion.Pdf.PdfDocument pdfDoc = converter.Convert(xpsStream);
                
            //    pdfDoc.Save(pdfFile);
            //    pdfDoc.Close(true);
            //}
        }
        private void SaveScreenAsPdf1(UIElement element, string pdfPath)
        {
            try
            {
                // Render UIElement to a bitmap
                var width = (int)element.RenderSize.Width;
                var height = (int)element.RenderSize.Height;
                const double dpi = 96; // same as RenderTargetBitmap
                ...
                using (var ms = new MemoryStream())
                {
                    ...
                    page.Width = width * 72 / dpi;
                    page.Height = height * 72 / dpi;
